...
int main(int argc, char *argv[]){
	...
	check = pthread_mutex_init(&m_ex_red, NULL);   
	error_check(check,1);
	check = pthread_mutex_init(&m_ex_blue, NULL); 
	error_check(check,1);
	...
	while(1) {
		...
		check = pthread_mutex_lock(&m_ex_red);  
		error_check(check,2);
		check = pthread_mutex_lock(&m_ex_blue);  
		error_check(check,2);
		...
		for(i=0; i<i_red; i++){
			...
			if(check!=0) {
				...
				free(array_r);
				free(array_b);
				check = pthread_mutex_destroy(&m_ex_red);
				error_check(check,4);
				check = pthread_mutex_destroy(&m_ex_blue);
				error_check(check,4);
		...
		while(1) {
			...
			for(i=0; i<N && i<(i_red-sum); i++ ) {
				check = pthread_mutex_unlock(&m_ex_red); //energopoioume ena amaksi na mpei sth gefyra
				error_check(check,3);
			...
			for(i=0; i<N && i<(i_blue-sum); i++ ) {
				check = pthread_mutex_unlock(&m_ex_blue);  //energopoioume ena amaksi na mpei sth gefyra
				error_check(check,3);
				check = pthread_mutex_lock(&safe_mtx);	//perimenoume na mas eidopoihsei to amaksi oti mphke sth gefyra
				error_check(check,2);
			}
			...
			if(un_count_r == i_red && un_count_b == i_blue){	
				...
				error_check(check,3);
				check = pthread_mutex_unlock(&m_ex_blue);
				error_check(check,3);
				check = pthread_mutex_unlock(&m_ex_red);
				error_check(check,3);
...
void *red(void *num_r){
	...
	check = pthread_mutex_lock(&m_ex_red); 	//kleidonoyme to amaksi mexri na to energopoihsei h main gia na mpei sth gefyra
	error_check(check,2);
...
void *blue(void *num_b){
	...
	check = pthread_mutex_lock(&m_ex_blue);  //kleidonoyme to amaksi mexri na to energopoihsei h main gia na mpei sth gefyra
	error_check(check,2);
	check = pthread_mutex_unlock(&safe_mtx);	//leme sth main oti to amaksi mphke sth gefyra
	error_check(check,3);
