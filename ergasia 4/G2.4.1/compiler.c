...
/*an to local size einai mhden kai to argument einai mhden tote den exoume argument*/
/*th for sta free*****************************/
#include <stdio.h>
#include <fcntl.h>
#include <unistd.h>
...
int main(int argc, char *argv[]) { 
	int fd_hex, fd_new, check, i, temp, k, j, p;
	/*Header*/
	...
	fd_new = open("new.txt", O_RDWR | O_CREAT, 0777 );
	if(fd_new<0){
		printf("Error with fd_new\n");
		close(fd_hex);
		return(-1);
	}
...
// 		printf("stop %d block %d sleep %d num_of_tasks %d\n",count_stop,count_blc,count_slp,num_of_tasks);
