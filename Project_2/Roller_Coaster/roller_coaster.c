...
int main(int argc, char *argv[]) {
	...
	if(check!=0) {
		...
		check = pthread_mutex_destroy(&ride);
		error_check(check,4);
		check = pthread_mutex_destroy(&roller);
		error_check(check,4);
		check = pthread_mutex_destroy(&mtx_rc);
		error_check(check,4);
		check = pthread_mutex_destroy(&mtx_pass);
		error_check(check,4);
		check = pthread_mutex_destroy(&msg_pass);
		error_check(check,4);
		check = pthread_mutex_destroy(&msg_rc);
		error_check(check,4);
